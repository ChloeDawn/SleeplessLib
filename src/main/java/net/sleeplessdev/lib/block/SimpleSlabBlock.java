...
public abstract class SimpleSlabBlock extends SimpleBlock {

    public static final PropertyEnum<Variant> VARIANT = PropertyEnum.create("variant", Variant.class);

    public SimpleSlabBlock(BlockMaterial material, ColorVariant color) {
        super(material, color);
        useNeighborBrightness = true;
        setLightOpacity(0);
    }

    public SimpleSlabBlock(BlockMaterial material) {
        super(material);
        useNeighborBrightness = true;
        setLightOpacity(0);
    }
    ...
    public boolean shouldSideBeRendered(IBlockState state, IBlockAccess world, BlockPos pos, EnumFacing side) {
        ...
        if (side.getAxis().isHorizontal()) {
            if (target.getBlock() instanceof SimpleSlabBlock) {
                if (target.getPropertyKeys().contains(VARIANT)) {
